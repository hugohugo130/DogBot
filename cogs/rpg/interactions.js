...
import {
    getNowPlayingEmbed,
} from "../../slashcmd/music/nowplaying.js";
import {
    getQueue,
...
目前的設定為：\`${text}\``)
            ...
            case "job_choose": {
                ...
                */
                ...
                const confirm_button = new ButtonBuilder()
                    .setCustomId(`job_confirm|${user.id}|${job_id}`)
                    .setLabel("我確定")
            ...
            case "refresh": {
                const [feature, ...args] = otherCustomIDs;
                ...
                switch (feature) {
                    ...
                    case "/info message": {
                        ...
                        const message = guild
                            ? await get_message_by_guild(guild, message_id)
                            : channel && await get_message_by_channel(channel, message_id);

                        if (!message) {
                            await interaction.followUp({
                                content: "找不到訊息，請確認訊息是否被刪除了。",
                                flags: MessageFlags.Ephemeral,
                            });
